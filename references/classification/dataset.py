...
class ClsDataset(Dataset):
    ...
    # def _parse_spe_anno(self, anno_filename):
    #     """Parse annotation file in spe format for each sample."""
    #     # remove .json
    #     img = anno_filename[:-5]
    #     with open(osp.join(self.annos_file, anno_filename), 'r') as f:
    #         spe_anno = json.load(f)
    #     if spe_anno.get('valid', False):
    #         step_1 = spe_anno.get('step_1', False)
    #         if step_1 and step_1.get('toolName', None) == 'tagTool':
    #             # only take the first result
    #             label_name = list(
    #                 step_1['result'][0]['result'].values())[0].split(';')[0]
    #             label = self.label_dict[label_name]
    #             self.data_info.append((img, label))
    #         else:
    #             warnings.warn(f'Invalid sensebee json: {anno_filename}.')
